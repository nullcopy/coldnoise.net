...
(USD) in a trustless, censorship-resistant way. Unlike traditional exchanges
that require KYC verification and can freeze accounts, this method combines
[Zashi](https://electriccoin.co/zashi) (a privacy-focused Zcash wallet) and
[zkP2P](https://zkp2p.xyz) (a peer-to-peer exchange) to purchase ZEC without
trusting a centralized intermediary.

The process is simple: use zkP2P to convert fiat (Venmo, Paypal, CashApp, etc.)
into USDC, then use Zashi to swap that USDC for ZEC. For more technical
details, see [how NEAR intents work in
...
2) Create an account
  ...
  > concerns. The email method is hosted by Privy relies on them to safeguard
  > your keys. For this reason, I would only use this account for small
...
Now that you have USDC on the Base blockchain, you'll use Zashi's built-in swap
functionality to convert it to Zcash (ZEC). Zashi uses NEAR-intents to
facilitate decentralized, trust-minimized swaps while maintaining privacy. The
ZEC you receive will be deposited directly into a shielded address in your
Zashi wallet.
...
4) Enter your Base address from zkP2P as a refund address in case the swap
fails
  > **Recommended**: Save this Base address as a contact in Zashi's address
  > book for easy access in future swaps
...
address Zashi provided. Wait for the transaction to be confirmed on the
blockchain
...
Remember to keep your Zashi wallet and recovery phrase secure. It will take a
few minutes for your ZEC to become spendable.

Now that you've got ZEC in a shielded address, why not test it out? Send me an
encrypted message using Zashi's memo feature. Scan the QR code below or copy
the address, attach a memo with your thoughts on this tutorial, and fire away.
